...
### Added
- **Public v2.2 SEO engine refresh**: synchronized the bundled SEO engine with the Daniel Agrici public `AgriciDaniel/claude-seo` v2.2 baseline, including 25 sub-skills, 18 portable sub-agents, Codex profiles, execution scripts, extension mirrors, security fixes, and knowledge-currency updates.
- **Runtime picker**: added local Codex, Claude Code, and Gemini CLI runtime selection while keeping Codex as the packaged default.
- **Guild Ledger prompt queue and steering**: added queued follow-up prompts, front-of-queue steering, active-turn controls, bridge health readbacks, and structured remote-control events for browser-owned setup, Hall, Battle, stop, clear, and vanquish flows.
- **Proof-backed remote control**: added structured browser and desktop proof recorders, bidirectional session ledger persistence, and CLI helpers for repeatable UI intent receipts.

### Changed
- **Version consolidation**: consolidated the public v2.2 release line under the `v2.2` tag so website and GitHub release surfaces no longer imply patch-level parity with the upstream Claude SEO project.
- **Runtime-neutral positioning**: documentation, installer output, app metadata, and runtime copy describe SEO Dungeon as Codex-default and compatible with optional local Claude Code and Gemini CLI workflows.
- **Env-first audit policy**: audit prompts and integration docs prefer project-local API/script access from configured credentials, with MCP servers treated as optional adapters.
- **UX and layout polish**: hardened YOLO arming, title inputs, project-folder recovery, prompt composer behavior, ledger resizing, hidden-ledger layout, and character scaling across the title, Gate, Hall, and Battle flows.

### Fixed
- **Windows local CLI launching**: resolved PATH-ordered PowerShell, CMD, BAT, EXE, Codex, Claude, and Gemini shims before spawn so long prompts remain intact and Windows package-manager shims do not fail with `EPERM`.
- **Runtime cache and bridge isolation**: cached audits now include runtime/profile keys, cleanup is scoped to the owning browser connection, and probes or reconnects cannot cancel another active turn.
- **Queue and steering safety**: queued prompts remain distinct until submitted or steered, Escape cannot accidentally stop active work, failed steering keeps prompts in place, and stale bridge states explain missing capabilities.
- **Security and audit hygiene**: hardened URL safety, bridge origin policy, failed-audit redaction, dependency state, and CI proof gates for the v2.2 release.

### Quality
- Validated across focused CLI launcher tests, live bridge tests, dialogue and UX harnesses, stress loops, build checks, browser remote-control proofs, and desktop proof recording before release consolidation.
